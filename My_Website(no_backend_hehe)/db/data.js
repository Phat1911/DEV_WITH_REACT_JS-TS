...
export const CtData = createContext();
...
const Data = () => {
  ...
  const opt = new Set(["All product"]);
  const cates = new Set(['All']);
  ...
  const [State, setState] = useState(Array(data.length).fill('inline-block')); 

  data.map((item) => {
      opt.add(item.company);
      cates.add(item.category);
      colors.add(item.color);
  })

  return <CtData.Provider value={{ data, ListShoes, setListShoes, opt, price, colors, cates, Category, setCate, State, setState, Price, setPrice, Color, setColor, Rec, setRec, Name, setName}}>
    <TopBar />
    <SideBar />
    <Body />
  </CtData.Provider>
